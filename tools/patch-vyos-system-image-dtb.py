...
def _copy_board_dtb_from_update_iso(
    ...
    native_profile = profile_path.with_name('boot-provider.json')
    native_metadata = None
    if native_profile.is_file():
        current_boot = _load_board_update_json(native_profile, 'installed boot provider')
        native_metadata = _load_board_update_json(
            iso_root / 'live' / 'boot-provider.json', 'update boot provider')
