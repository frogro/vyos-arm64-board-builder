#!/bin/vbash
# Optimized ARM64 variant v5.18: apply ModemManager bearer runtime IPv4/routing before bound data-path validation and persist complete WWAN failover state.
...
#
# modem-connect.sh
# Universal modem setup for VyOS on ARM64 systems.
#
...
#
# Project: https://github.com/frogro/vyos-arm64-board-builder
#
set -o pipefail
...
AP_IF="${AP_IF:-auto}"
AP_IF_CACHE="${AP_IF_CACHE:-/config/vyos-ap-interface.conf}"
...
if [ "$SERVICE_RUN" -eq 1 ]; then
  ...
  # Tested ARM64 policy: the USB/RNDIS FM350 uses the kernel's native eth1 name.
...
fm350_install_rndis_recovery() {
  ...
  # The tested ARM64 system setup creates the FM350 RNDIS interface as eth1
  ...
  # left entirely to the kernel and naturally come up as eth1 on the tested ARM64 system setup.
...
if printf '%s\n' "$ACTIVE_BEFORE" | grep -Fq "set firewall ipv4 name VYOS-WAN-IN "; then
...
vyos_desired_config_active() {
  ...
  printf '%s\n' "$active_cfg" | grep -Fq "set firewall ipv4 name VYOS-WAN-IN " && ap_fw_present=1
  if [ "$WWAN_CONNECTED" -eq 1 ]; then
    nat_ok=0
    route_cfg_ok=0
    ...
    if [ "$ap_fw_present" -eq 1 ]; then
      ...
      if printf '%s\n' "$active_cfg" | grep -Fq "set firewall ipv4 forward filter rule $WWAN_FORWARD_RULE inbound-interface name '$NET_IF'" && \
         printf '%s\n' "$active_cfg" | grep -Fq "set firewall ipv4 forward filter rule $WWAN_FORWARD_RULE jump-target 'VYOS-WAN-IN'"; then
        firewall_ok=1
      fi
    fi
  else
