...
for file in ustreamer ustreamer-dump LICENSE build.env SHA256SUMS; do
    [[ -s "$ARTIFACTS/$file" ]] || {
...
if [[ "$(uname -m)" == "aarch64" ]]; then
    missing="$(chroot "$ROOTFS" ldd /usr/local/bin/ustreamer | grep 'not found' || true)"
    [[ -z "$missing" ]] || {
        echo "ERROR: µStreamer runtime dependency missing:" >&2
        echo "$missing" >&2
        exit 1
    }
fi
