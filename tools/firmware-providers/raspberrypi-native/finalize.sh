...
BOARD="${1:?Usage: $0 <board> <firmware-mount> <version-dir> <artifacts> <grub-version-cfg>}"
FIRMWARE_MNT="${2:?Usage: $0 <board> <firmware-mount> <version-dir> <artifacts> <grub-version-cfg>}"
VERSION_DIR="${3:?Usage: $0 <board> <firmware-mount> <version-dir> <artifacts> <grub-version-cfg>}"
ARTIFACTS="${4:?Usage: $0 <board> <firmware-mount> <version-dir> <artifacts> <grub-version-cfg>}"
GRUB_VERSION_CFG="${5:?Usage: $0 <board> <firmware-mount> <version-dir> <artifacts> <grub-version-cfg>}"
...
python3 - "$FIRMWARE_MNT/config.txt" "$FIRMWARE_MNT/cmdline.txt" "$GRUB_VERSION_CFG" <<'PY'
from pathlib import Path
import re
import sys

config_path, cmdline_path, grub_path = map(Path, sys.argv[1:])
